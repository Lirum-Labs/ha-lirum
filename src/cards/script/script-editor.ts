...
import type { ScriptConfig } from './script-card';
...
@customElement(CARDS.script.editor)
export class LirumScriptEditor extends LitElement {
  ...
  private _changed(e: CustomEvent): void {
    this.dispatchEvent(
      new CustomEvent('config-changed', {
        detail: { config: (e.detail as { value: ScriptConfig }).value },
        bubbles: true,
        composed: true,
      }),
    );
