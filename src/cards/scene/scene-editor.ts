...
import type { SceneConfig } from './scene-card';
...
@customElement(CARDS.scene.editor)
export class LirumSceneEditor extends LitElement {
  ...
  private _changed(e: CustomEvent): void {
    this.dispatchEvent(
      new CustomEvent('config-changed', {
        detail: { config: (e.detail as { value: SceneConfig }).value },
        bubbles: true,
        composed: true,
      }),
    );
